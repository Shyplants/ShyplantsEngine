    ...
    const XMMATRIX& viewProj)
{
    ...
    XMMATRIX T_offset = XMMatrixTranslation(m_renderOffset.x, m_renderOffset.y, 0.f);
    
    XMMATRIX T_pivot = XMMatrixTranslation(-pivotPixel.x, -pivotPixel.y, 0);

    XMMATRIX scale = XMMatrixScaling(spriteW * m_scale.x, spriteH * m_scale.y, 1.f);

    XMMATRIX finalWorld = scale * T_pivot * T_offset * world;

    // -------------------------------------------------
    // Constant Buffer (CPU cache)
    // -------------------------------------------------
